...
<body>
    <div class="admin-header">
        <h1>🛡️ Admin Panel - Edit Alert</h1>
        <div>
            ...
            <form method="POST" action="{{ route('admin.alerts.update', $alert->id) }}">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="title" name="title" class="form-input" 
                           placeholder="e.g., Flood Warning - Dhaka" value="{{ old('title', $alert->title) }}" required>
                    @error('title')
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select id="type" name="type" class="form-select" required>
                            ...
                            <option value="Cyclone" {{ old('type', $alert->type) == 'Cyclone' ? 'selected' : '' }}>🌪️ Cyclone</option>
                            <option value="Fire" {{ old('type', $alert->type) == 'Fire' ? 'selected' : '' }}>🔥 Fire</option>
                            <option value="Health Emergency" {{ old('type', $alert->type) == 'Health Emergency' ? 'selected' : '' }}>🏥 Health Emergency</option>
                            <option value="Other" {{ old('type', $alert->type) == 'Other' ? 'selected' : '' }}>⚠️ Other</option>
                        </select>
                        @error('type')
                            <div class="error">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <select id="severity" name="severity" class="form-select" required>
                            ...
                            <option value="Moderate" {{ old('severity', $alert->severity) == 'Moderate' ? 'selected' : '' }}>🟡 Moderate</option>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="location" name="location" class="form-input" 
                           placeholder="e.g., Dhaka Metropolitan Area" value="{{ old('location', $alert->location) }}" required>
                    @error('location')
                        <div class="error">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="number" id="latitude" name="latitude" class="form-input" 
                               step="0.000001" placeholder="e.g., 23.8103" value="{{ old('latitude', $alert->latitude) }}">
                        @error('latitude')
                            <div class="error">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" id="longitude" name="longitude" class="form-input" 
                               step="0.000001" placeholder="e.g., 90.4125" value="{{ old('longitude', $alert->longitude) }}">
                        @error('longitude')
                            <div class="error">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="form-group">
                    <label class="form-label" for="status">Alert Status *</label>
                    <select id="status" name="status" class="form-select" required>
                        <option value="Active" {{ old('status', $alert->status) == 'Active' ? 'selected' : '' }}>✅ Active</option>
                        <option value="Resolved" {{ old('status', $alert->status) == 'Resolved' ? 'selected' : '' }}>✅ Resolved</option>
                        <option value="Monitoring" {{ old('status', $alert->status) == 'Monitoring' ? 'selected' : '' }}>👁️ Monitoring</option>
                    </select>
                    @error('status')
                        <div class="error">{{ $message }}</div>
                    @enderror
                ...
                <div class="form-group">
                    ...
                    <textarea id="description" name="description" class="form-textarea" 
                              placeholder="Provide detailed information about the alert, safety instructions, and recommended actions..." required>{{ old('description', $alert->description) }}</textarea>
                    @error('description')
                        <div class="error">{{ $message }}</div>
                    @enderror
                </div>
